...
public class MyDB extends SQLiteOpenHelper
{
	...
	public void onCreate(SQLiteDatabase db){
		...
		db.execSQL("CREATE TABLE " + TB_MESSAGE + 
				   ...
				   "type VARCHAR(32), send_time INT, head VARCHAR(2048), status INT)");
	...
	public void saveMessage(List<MessageData> data) {
		...
		for (MessageData d: data) {
			...
			val.put("type", d.type);
			val.put("head", d.head);
			val.put("status", ItemBeanChat.DONE);
			db.insert(TB_MESSAGE, null, val);
		}
		db.close();
	}
	
	public void saveMessage(List<ItemBeanChat> data) {
		SQLiteDatabase db = getWritableDatabase();
		for (ItemBeanChat d: data) {
			ContentValues val = new ContentValues();
			val.put("mid", d.mid);
			val.put("gid", d.gid);
			val.put("send_time", d.send_time);
			val.put("username", d.username);
			val.put("text", d.message);
			val.put("type", d.type);
			val.put("head", d.head_url);
